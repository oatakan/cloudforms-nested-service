...
def override_service_name(dialog_options_hash)
  ...
  new_service_id = dialog_options_hash.fetch(:service, nil)
  
  new_service = $evm.vmdb('service', new_service_id) rescue nil
  if new_service.nil?
    $evm.log(:info, "Can't find service with ID: #{new_service_id}")
    new_parent_service_name = dialog_options_hash.fetch(:service_name, nil)
    new_parent_service_name = "#{@service.name}-#{Time.now.strftime('%Y%m%d-%H%M%S')}" if new_parent_service_name.blank? 
    $evm.log(:info, "Creating a new parent service: #{new_parent_service_name}")
    
    new_parent_service = $evm.vmdb('service').create(:name => "#{new_parent_service_name}")
    new_parent_service.display = true
    
    $evm.log(:info, "Adding Service #{$evm.root['service'].name} to #{new_parent_service.name}")
    $evm.root['service'].parent_service = new_parent_service
  else
    $evm.log(:info, "Found service with ID: #{new_service_id}")
    $evm.log(:info, "Adding Service #{$evm.root['service'].name} to #{new_service.name}")
    $evm.root['service'].parent_service = new_service
  end


  role_tag = $evm.root['dialog_tag_0_role']
  server_role_name = ''
  if !role_tag.length.zero?
    $evm.log(:info, "Role tag = \'#{role_tag}\'")
    server_role_name = role_tag.first.name
  end
  
  if server_role_name.blank?
    new_service_name = "#{@service.name}-#{Time.now.strftime('%Y%m%d-%H%M%S')}"
  else
    new_service_name = "#{server_role_name}-#{Time.now.strftime('%Y%m%d-%H%M%S')}"
  end
